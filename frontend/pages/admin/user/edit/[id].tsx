...
function UserEditScreen() {
	...
	const { users: userInfo, loading, success } = useSelector((state: RootState) => state.userInfo);
	...
	useEffect(() => {
		if (success) {
			...
			setValue('isAdmin', userInfo?.isAdmin);
		}
	}, [success, router, setValue, dispatch, userId, userInfo._id]);
	...
	const submitHandler = async (data: any, e) => {
		...
		try {
			dispatch(updateUser({ _id: userId, isAdmin: isAdmin, data }));
			setEdit(!edit);
...
export default dynamic(() => Promise.resolve(UserEditScreen), {
	ssr: false,
});
